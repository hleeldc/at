...
class _TableRowProxy(object):
    ...
    def __setitem__(self,i,v):
        ...
        self.tab.emitter.emit(QtCore.SIGNAL("cellChanged"),
                              self.num,i,self.data[i],w)
...
class _TableModelProxy(object):
    ...
    def setHeader(self, col, header):
        colname, coltype = header
        self.super.setColumnHeader(col, colname, coltype)
        self.emitter.emit(QtCore.SIGNAL("setHeader"),col,header)
    ...
    def insertRow(self,i=None,row=None):
        ...
        if self.super.insertRow(i,row):
            for k in range(i,len(self.table)):
                self.table[k].num = k
            self.emitter.emit(QtCore.SIGNAL("insertRow"),i,self.table[i])
    ...
    def insertColumn(self,i=None,col=None):
        if self.super.insertColumn(i,col):
            self.emitter.emit(QtCore.SIGNAL("insertColumn"),i,col)
    ...
    def takeRow(self,i):
        ...
        if i == self.selection:
            self.selection = None
        self.emitter.emit(QtCore.SIGNAL("takeRow"),i,r)
        if not self.undoStackReadOnly:
    ...
    def takeColumn(self,i):
        ...
        self.emitter.emit(QtCore.SIGNAL("takeColumn"),i)
        if i == self.selection:
            self.selection = None
    ...
    def sort(self, *args):
        ...
        for i,row in enumerate(self.table):
            row.num = i
        self.emitter.emit(QtCore.SIGNAL("sort"))

    def select(self, sel):
        self.selection = sel
        self.emitter.emit(QtCore.SIGNAL("select"),sel)

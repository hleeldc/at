    ...
    def exportLPathTable(self, TableModel, sid=0, tid=0):
        ...
        for r in TAB:
            ...
            for k,v in r['attributes']:
                ...
                row2 = TAB2[-1]
                row2['type'] = 'att'
                row2['name'] = k.encode('utf-8')
                row2['value'] = v.encode('utf-8')
        return TAB2
    ...
    def importLPathTable(cls, table):
        ...
        for row in tab:
            if row['type'] == 'att':
                ...
                if f == '@label':
                    node = cls()
                    node.setData('label', v)
                    node.setData('type', row['type'])
                    node.setData('id', row['id'])
                    T[-1].attach(node)
                else:
                    if f in T[-1].data:
                        T[-1].getData(f).append(v)
                    else:
                        T[-1].setData(f, [v])
            else:
                p = row['pid']
